...
def pair_create(classes):
    ...
    for x in range(len(classes)):
        for i in range(len(classes[x])):
            ...
            pairs.append([classes[x][i],classes[x][i-k],classes[j][k]])
            labels.append([1,1])
    return pairs,labels
...
def distance(vector):
    ...
    vector_a,vector_p,vector_n = vector[0],vector[1],vector[2]
    #print(vector_a)
    #print(vector_p)
    margin=0.2
    ...
    sum1=k.sum(k.square(vector_a-vector_n),axis=1)
    
    sumn=tf.reshape(sum1,[-1,1])
    ...
    return sump-sumn+margin

def triplet_loss(labels,loss):
    from keras import backend as k
    #import tensorflow as tf
...
inputn=Input(shape=(32,32,3))

# creates feature vector for 3 different inputs but with weights being shared
vector_a=model(inputa)
vector_p=model(inputp)
vector_n=model(inputn)
loss=Lambda(distance)([vector_a,vector_p,vector_n])
print(loss)  
model1=Model(inputs=[inputa,inputp,inputn],outputs=loss)
...
model1=load_model('Siamese2.h5',custom_objects={'triplet_loss': triplet_loss,'accuracy_siamese':accuracy_siamese})

adam1=adam(lr=0.000001)
model1.compile(optimizer=adam1,
              loss=triplet_loss,
              metrics=['accuracy', accuracy_siamese]
              ) 
details=model1.fit([pairs[:,0],pairs[:,1],pairs[:,2]],labels,batch_size=16,epochs=25)
...
test_loss=model1.predict([pairs[:,0],pairs[:,1],pairs[:,0]])
